...
(function () {
  ...
  function bootstrap(tries = 1) {
    if ((/discuss/.test(location.href) && $("#control_bar_handler").css("visibility") === "visible") || (typeof W != "undefined" && W && W.map && W.model && W.loginManager.user && $ && W.model.getTopState() && $(".app.container-fluid.show-sidebar").length > 0)) {
      preinit();
    } else if (tries < 1000)
      setTimeout(function () {
        bootstrap(++tries);
  ...
  function getCurrentState() {
    if (W.model.getTopState().attributes === undefined) return W.model.getTopState().getName();
  ...
  function BuildWazebar() {
    ...
    if (WazeBarSettings.NAServerUpdate) {
      GM_xmlhttpRequest({
        ...
        onload: function (response) {
          ParseStatusFeed(response, "NA", "WazebarStatus");
        },
      });
    }
    ...
    if (WazeBarSettings.ROWServerUpdate) {
      GM_xmlhttpRequest({
        ...
        onload: function (response) {
          ParseStatusFeed(response, "ROW", "WazebarStatusROW");
        },
      });
    }
  ...
  function ParseStatusFeed(data, updateType, targetId) {
    let regex;
    if (updateType === "NA") {
      regex = /North America map tiles were successfully updated to: (.*?)<\/title>/;
    } else if (updateType === "ROW") {
      regex = /International map tiles were successfully updated to: (.*?)<\/title>/;
    }

    const result = data.responseText.match(regex);

    if (result) {
      const dateString = result[1].trim();
      const date = new Date(dateString);
      const formattedDate = formatDate(date);

      $("#" + targetId).append(formattedDate);
    } else {
      $("#" + targetId).append(" No updates found.");
    }
  }
    ...
    Object.keys(WazeBarSettings.header).forEach(function (state) {
      if (WazeBarSettings.header[state].unlock) {
        stateUnlocks +=
          '<div class="WazeBarText WazeBarForumItem" id="' +
          state.replace(" ", "_") +
          'Unlock"><a href="https://www.waze.com/discuss/search?q=' +
          state +
          '%20%23united-states%3Aus-unlock-and-update-requests%20order%3Alatest" ' +
          LoadNewTab() +
          ">" +
  ...
  function BuildSettingsInterface() {
    ...
    $section.html(
      [
        ...
        "<div style='display: flex; align-items: center; gap: 8px;'>",
        "<input type='color' id='colorPickerForumFont' style='width: 60px;' value='" + WazeBarSettings.ForumFontColor + "'/>",
        "<label for='colorPickerForumFont'>Forum Links Color</label>",
        "</div>",
        // Wiki font color with default value
        "<div style='display: flex; align-items: center; gap: 8px;'>",
        "<input type='color' id='colorPickerWikiFont' style='width: 60px;' value='" + WazeBarSettings.WikiFontColor + "'/>",
        ...
        // Forum History
        "<div style='display: flex; align-items: center; gap: 8px;'>",
        "<input type='number' id='forumHistory' min='1' style='width: 60px;' value='" + WazeBarSettings.forumHistory + "'/>",
    ...
    $("#btnWazebarImportSettings").click(function () {
      ...
      if (inputSettings === "") {
        WazeWrap.Alerts.error(SCRIPT_NAME, "Import Settings Input string is empty.");
        return;
      }

      if (!isValidJson(inputSettings)) {
        WazeWrap.Alerts.error(SCRIPT_NAME, "Import Settings has Invalid JSON format.");
        return;
      }
      ...
      try {
        ...
        if (typeof parsedSettings === "object" && parsedSettings !== null) {
          ...
          WazeWrap.Alerts.info(SCRIPT_NAME, "Settings imported successfully. Please review and Save.");
        } else {
          WazeWrap.Alerts.error(SCRIPT_NAME, "Valid JSON, but the format is not suitable for WazeBar settings.");
        }
      } catch (e) {
    ...
    $("#btnWazebarCopySettings").click(function () {
      ...
      WazeWrap.Alerts.info(SCRIPT_NAME, "Your settings have been copied to the clipboard.");
    });
  ...
  function injectCss() {
    var css = [
      ...
      ".WazeBarText { display: inline; padding-right: 4px; margin-left: 4px; border-right: thin solid grey; font-size: " + WazeBarSettings.BarFontSize + "px; }",
      ...
      ".WazeBarIcon { display: inline; margin-left: 8px; cursor: pointer; font-size: " + WazeBarSettings.BarFontSize + "px; }",
      ...
      "#WazeBarFavoritesAddContainer input { height: 25px; border: 1px solid #000000; padding: 4px; border-radius: 6px; }",
      ...
      ".WazeBarUnreadList.unread-item:hover { background: #e1e1e1; }",
      ...
      ".flex-column { padding: 8px; position: relative; box-sizing: border-box; border: 1px solid #ccc; background-color: #f9f9f9; min-width: 200px;  max-width: 250px; flex: 1 1 auto; min-height: 510px; }",
      ...
      "#txtWazebarSettings, #txtWazebarImportSettings { width: 100%; height: auto; min-height: 100px; max-height: 500px; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; box-sizing: border-box; resize: vertical; }",
      ...
      ".styled-select { width: 220px; font-size: 14px; line-height: 0.5; height: 30px; padding: 1px; border: 1px solid #ccc; border-radius: 6px; background-color: #fff; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); }",
